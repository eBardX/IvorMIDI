...
public enum MIDISystemMessage {
    ...
    /// Creates a `MIDISystemMessage` instance from its MIDI wire encoding, or
    /// `nil` if the bytes do not encode a valid system message.
    ///
    /// - Parameter statusByte: The status byte (0xF0–0xFF).
    /// - Parameter dataBytes:  The data bytes. For a system exclusive message,
    ///                         these include the terminating EOX byte (0xF7).
...
extension MIDISystemMessage {
    ...
    /// The status byte of this message’s MIDI wire encoding.
