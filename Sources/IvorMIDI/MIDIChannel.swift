...
extension MIDIChannel: MIDIBytesConvertible {

    // MARK: Public Initializers

    /// Creates a `MIDIChannel` instance from its MIDI wire encoding, or `nil`
    /// if the bytes do not encode a valid channel number.
    ///
    /// - Parameter bytesValue: A single byte holding the zero-based channel
    ///                         number (0–15).
    ...
    /// The MIDI wire encoding of this channel number: a single byte holding
    /// the zero-based channel number (0–15).
...
extension MIDIChannel: UIntRepresentable {
}
