...
extension MIDIController: MIDIBytesConvertible {

    // MARK: Public Initializers

    /// Creates a `MIDIController` instance from its MIDI wire encoding, or
    /// `nil` if the bytes do not encode a valid controller number.
    ///
    /// - Parameter bytesValue: A single byte holding the controller number
    ///                         (0–127).
    ...
    /// The MIDI wire encoding of this controller number: a single byte
    /// holding the controller number (0–127).
...
extension MIDIController: UIntRepresentable {
}
