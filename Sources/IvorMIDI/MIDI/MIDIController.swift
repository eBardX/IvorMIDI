...
extension MIDIController: BytesValueConvertible {
    ...
    /// Creates a `MIDIController` instance from the provided array of
    /// bytes, or `nil` if the bytes do not represent a valid controller
    /// number.
    ///
    ...
    /// The array of bytes representing this controller number, or `nil` if
    /// the controller number cannot be represented as a single byte.
