...
public enum MIDIChannelMessage {
    ...
    /// Creates a `MIDIChannelMessage` instance from its MIDI wire encoding,
    /// or `nil` if the bytes do not encode a valid channel message.
    ///
    /// - Parameter statusByte: The status byte (0x80–0xEF), combining the
    ///                         message type and the zero-based channel number.
    /// - Parameter dataBytes:  The data bytes. Their count must match the
    ///                         count expected for the status byte.
...
extension MIDIChannelMessage {
    ...
    /// Returns the number of data bytes that follow the provided status byte
    /// in a channel message.
    ///
    /// - Parameter statusByte: The status byte.
    ///
    /// - Returns:  The expected number of data bytes (1 or 2), or `nil` if the
    ///             status byte is not a channel message status byte.
    ...
    /// The data bytes of this message’s MIDI wire encoding, or `nil` if it
    /// cannot be encoded.
    ...
    /// The status byte of this message’s MIDI wire encoding, or `nil` if it
    /// cannot be encoded.
    ...
    internal static func isModeMessage(_ statusByte: UInt8,
    ...
    internal static func isVoiceMessage(_ statusByte: UInt8,
                                        _ dataBytes: [UInt8]) -> Bool {
    ...
    private func _combine(_ value1: any MIDIBytesConvertible,
                          _ value2: any MIDIBytesConvertible) -> [UInt8]? {
