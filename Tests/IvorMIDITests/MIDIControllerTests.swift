...
extension MIDIControllerTests {
    ...
    @Test
    func codable() throws {
        let original: MIDIController = 7
        let data = try JSONEncoder().encode(original)
        let decoded = try JSONDecoder().decode(MIDIController.self,
                                               from: data)
    ...
    func codable_invalidValueThrows() {
        ...
        #expect(throws: DecodingError.self) {
            try JSONDecoder().decode(MIDIController.self,
                                     from: data)
        }
    }
    ...
    @Test
    func comparable() {
        let low: MIDIController = 7
        let high: MIDIController = 64
    ...
    @Test
    func description() {
        let value: MIDIController = 7

        #expect(value.description == "7")
    }

    @Test
    func equality() {
        let value1: MIDIController = 7
        let value2: MIDIController = 7
        let value3: MIDIController = 64
    ...
    @Test
    func hashable() {
        let set: Set<MIDIController> = [7, 7, 64]
